...
export async function handleCreateOrder(req, res) {
  ...
  req.on("end", async () => {
    ...
    try {
      ...
      handleResponse(res, 201, { message: "Sipariş oluşturuldu", orderCode });
...
export async function handleGetOrderByCode(orderCode, res) {
  try {
    ...
    if (order) {
      ...
      res.end(JSON.stringify({ message: "Sipariş bulunamadı." }));
  ...
  req.on("end", async () => {
    ...
    try {
      ...
      if (result.modifiedCount > 0) {
        handleResponse(res, 200, { message: "Sipariş statüsü güncellendi." });
        logger.info(`Durum güncellendi. orderCode: ${orderCode}`);
      } else {
        handleResponse(res, 404, { message: "Sipariş bulunamadı." });
